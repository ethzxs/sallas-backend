...
const { encryptImapPassword } = require('../utils/imapCrypto.cjs');
...
router.post('/email-connections/:id/set-imap-password', async (req, res) => {
  ...
  try {
    ...
    if (existingErr) {
      return res.status(500).json({ error: 'Failed to fetch email connection', details: existingErr.message || existingErr });
    }
    if (!existing) {
    ...
    const { error } = await serviceClient
      ...
      .update({
        ...
        provider: existing.provider || 'imap',
        is_verified: true,
    ...
    if (error) {
      return res.status(500).json({ error: 'Failed to update email connection', details: error.message || error });
    }
...
router.put('/email-connections/:id', async (req, res) => {
  ...
  try {
    ...
    if (getErr) {
      return res.status(500).json({ error: 'Failed to fetch email connection', details: getErr.message || getErr });
    }
    if (!existing) {
      return res.status(404).json({ error: 'Email connection not found' });
    }
    ...
    if (typeof host !== 'undefined' && host !== existing.host) {
      updates.host = host;
      changed = true;
    }
    ...
    if (typeof port !== 'undefined') {
      ...
      if (Number.isFinite(newPort) && newPort !== oldPort) {
        updates.port = newPort;
        changed = true;
      }
    }

    if (typeof username !== 'undefined' && username !== existing.username) {
      updates.username = username;
      changed = true;
    }

    if (typeof mailbox !== 'undefined' && mailbox !== existing.mailbox) {
    ...
    if (updateErr) {
      return res.status(500).json({ error: 'Failed to update email connection', details: updateErr.message || updateErr });
    }
